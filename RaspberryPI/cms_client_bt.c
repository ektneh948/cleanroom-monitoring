...
char name[ID_MAX_COUNT]="[Default]";
char pwd[PW_MAX_COUNT]="PASSWD";
...
int main(int argc, char *argv[])
{
	...
	struct sockaddr_rc addr = { 0 };
  	char dest[18] = "98:DA:60:02:6B:43";	//iot00
	char msg[BUFFER_MAX_COUNT];

	if(argc != 5) {
		printf("Usage : %s <IP> <port> <name> <pwd>\n",argv[0]);
		exit(1);
	}

	sprintf(name, "%s",argv[3]);
	sprintf(pwd, "%s",argv[4]);
...
void * send_msg(void * arg)  //bluetooth --> server
{
	...
	while(1) {
		...
		{
			ret=read(dev_fd->btfd, msg+total,BUFFER_MAX_COUNT-total);
			if(ret > 0)
			{
				total += ret;
			}
...
void * recv_msg(void * arg)  //server --> bluetooth
{
	...
	while(1) {
		...
		str_len = read(dev_fd->sockfd, name_msg, ID_MAX_COUNT + PW_MAX_COUNT + BUFFER_MAX_COUNT );
		if(str_len <= 0) 
		{
			dev_fd->sockfd = -1;
			return NULL;
